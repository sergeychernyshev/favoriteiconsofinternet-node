...
async function generateTiles() {
  ...
  // Combine content into groups
  let allImagesHtml = '';

  for (let i = 0; i < allImageTags.length; i += CONFIG.GROUP_SIZE) {
    const groupTags = allImageTags.slice(i, i + CONFIG.GROUP_SIZE);
    allImagesHtml += `<div class="tile-group">\n${groupTags.join('\n')}\n</div>\n`;
  }
...
<html>
  <head>
    ...
    <style>
      :root {
        --tile-width: ${imageSize}px;
        --tile-map-width: round(up, 100vw, var(--tile-width));
        --tile-group-height: calc(${CONFIG.GROUP_SIZE * imageSize}px * (var(--tile-width) / var(--tile-map-width)));
      }
      * {
      ...
      .tiles-wrapper {
          ...
          width: var(--tile-map-width);
      }
      .tile-group {
        display: content;
        content-visibility: auto;
        contain-intrinsic-size: var(--tile-map-width) var(--tile-group-height);
      }
      img {
        border: 0;
        margin: 0;
